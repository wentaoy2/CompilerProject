...
operand assign_class::code(CgenEnvironment *env)
{
	...
	printer.store(exprOp, assignment);
	return exprOp;
}
...
operand cond_class::code(CgenEnvironment *env)
{
	...
	ValuePrinter printer(*(env->cur_stream));
	operand reg;
	string typeString = then_exp->get_type()->get_string();
	op_type type;
...
operand object_class::code(CgenEnvironment *env)
{
	...
	operand lookUp = *(env->lookup(name));
	return printer.load(lookUp.get_type().get_deref_type(),lookUp);
}
